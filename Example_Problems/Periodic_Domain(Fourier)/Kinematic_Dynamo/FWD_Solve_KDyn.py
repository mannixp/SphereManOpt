...
'''
Solve the kinematic dynamo optimisation problem

maximise  J(B0,U) = ⟨B_T , B_T⟩ ≡ 1/V ∫ B2 T dV, 
  B0,U

s.t. 	⟨B0, B0⟩ = 1, 
		⟨U , U ⟩ = 1, 
		∂B/∂t − ∇ × (U × B) + ∇Π − (1/Rm)∇^2 B = 0, 
		∇ · B = 0, 
		∇ · U = 0

in a periodic box L_x = L_y = L_z = 2π, using a Fourier basis.

To run this code :        mpiexec -np 4 python3 FWD_Solve_KDyn.py 
& to plot the solution :  python3 plot_figure_KDyn.py
'''
...
def Integrate_Field(domain,F):
	...
	KE(t) = 1/V int_v F(x,y,z) dV, where F = u^2 + v^2 + w^2 & dV = dx*dy*dz
	...
	# 1) Multiply by the integration weights 
...
def Inner_Prod_3(x,y,domain,random_arg=None):
...
def FWD_Solve_IVP_Prep(Bx0,Ux0, domain,Rm,dt,  N_ITERS):
	...
	Rm - float Magnetic Reynolds number
	dt - float numerical integration time-step
	N_ITERS - int total number of steps

	Returns:
		Smoothed initial conditions Bx0,Ux0
...
def FWD_Solve_IVP_Lin(X0, domain,Rm,dt,  N_ITERS,N_SUB_ITERS, X_FWD_DICT, Cost_function= "Final", Adjoint_type = "Discrete"):
	...
	Rm - float Magnetic Reynolds number
	dt - float numerical integration time-step
	N_ITERS,N_SUB_ITERS - int total number of steps
	X_FWD_DICT = {'b_x(x,t)','b_y(x,t)','b_z(x,t)'} - Python dictionary storing forwards solve
...
def ADJ_Solve_IVP_Lin(X0, domain,Rm,dt,  N_ITERS,N_SUB_ITERS, X_FWD_DICT, Cost_function= "Final", Adjoint_type = "Discrete"):	
	"""
	Integrates the initial condition X(t=0) = Bx0 -> B(x,T);
	using the induction equation,
	...
	Returns:
		Gradient of J = final magnetic energy/time integrated magnetic energy w.r.t. Bx0 & Ux0
...
if __name__ == "__main__":
	...
	_, dBx0, dUx  = Generate_IC(Npts,X_domain,M_0,Noise);
	dX_0 		  = [dBx0,0.*dUx]
	#Adjoint_Gradient_Test(X_0,dX_0,FWD_Solve_IVP_Lin,ADJ_Solve_IVP_Lin,Inner_Prod_3,args_f,args_IP,epsilon=1e-04)



	# 3) Optimise for fields Ux0,Bx0
	from Sphere_Grad_Descent import Optimise_On_Multi_Sphere, plot_optimisation
	RESIDUAL, FUNCT, X_opt = Optimise_On_Multi_Sphere(X_0,Constraints,FWD_Solve_IVP_Lin,ADJ_Solve_IVP_Lin,Inner_Prod_3,args_f,args_IP,max_iters=10, alpha_k = 100., LS = 'LS_wolfe', CG = True,callback=File_Manips)
